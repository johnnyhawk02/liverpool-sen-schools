...
try:
    with open('liverpool_schools.html', 'r', encoding='utf-8') as f:
        html_content = f.read()
except FileNotFoundError:
    print("Error: liverpool_schools.html not found.")
    exit()

# Remove the START/END markers if they are present in the file
html_content = html_content.replace("--- START OF FILE liverpool_schools.html ---", "").replace("--- END OF FILE ---", "").strip()
...
if results_list:
    ...
    for article in school_articles:
        ...
        if detailed_div:
            found_full_description = False
            
            # First approach: Look in details.field_section
            for details in detailed_div.select('details.field_section'):
                ...
                if any(term in summary_text.lower() for term in ['about', 'description', 'local offer', 'overview', 'information']):
                    # Look for description in dl/dt/dd structure
                    dl = details.find('dl')
                    if dl:
                        for dt in dl.find_all('dt'):
                            dt_text = dt.get_text(strip=True).lower()
                            if any(term in dt_text for term in ['description', 'about', 'overview', 'information', 'provision']):
                                dd = dt.find_next_sibling('dd')
                                if dd:
                                    full_description = dd.get_text(" ", strip=True)
                                    if full_description and len(full_description) > len(school.get('summary_description', '')):
                                        school['summary_description'] = full_description.replace('\n', ' ').strip()
                                        found_full_description = True
                                        break
                    
                    # If no dl structure, try to get content directly from the details
                    if not found_full_description:
                        # Exclude the summary element from the text extraction
                        summary_elem = details.find('summary')
                        if summary_elem:
                            summary_elem.extract()
                        
                        # Get all text from the details
                        full_text = details.get_text(" ", strip=True)
                        if full_text and len(full_text) > len(school.get('summary_description', '')):
                            school['summary_description'] = full_text.replace('\n', ' ').strip()
                            found_full_description = True
            
            # Second approach: If still not found, look for descriptions in other structures
            if not found_full_description:
                # Look for sections with description-like content
                for section in detailed_div.select('div.section'):
                    heading = section.find(['h3', 'h4', 'h5', 'strong'])
                    if heading and any(term in heading.get_text().lower() for term in ['about', 'description', 'overview']):
                        ...
                        for sibling in heading.next_siblings:
                            ...
                            if sibling.string:
                                full_text += " " + sibling.string.strip()
                            elif sibling.name:
                ...
                # If still no description, look for any paragraphs that might contain relevant info
                if not found_full_description:
                    paragraphs = detailed_div.select('p')
                    combined_text = " ".join([p.get_text(" ", strip=True) for p in paragraphs])
                    if combined_text and len(combined_text) > len(school.get('summary_description', '')):
                        school['summary_description'] = combined_text.replace('\n', ' ').strip()
                        found_full_description = True

        # Clean up any remaining truncated description
        if school['summary_description'] and (school['summary_description'].endswith('\u2026') or school['summary_description'].endswith('...')):
            # Try to find a more complete version in the full HTML
            # If we still have a truncated description, at least make the ellipsis consistent
            school['summary_description'] = re.sub(r'[\.…]+$', '...', school['summary_description'])
